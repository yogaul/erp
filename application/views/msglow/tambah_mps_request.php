        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <!-- DataTales Example -->
                    <form action="<?php echo base_url().'glow/simpan_mps' ?>" method="post" id="form-kalkulator">
                                    ...
                                    <div class="col-6">
                                        <div class="form-group">
                                            ...
                                            <select name="produk" class="form-control mselect" required="" id="produk-jadi-msglow">
                                                ...
                                                ?>
                                                <option value="<?= $value->id_msglow_request ?>"><?= $value->usulan_nama_produk ?> ( <?= number_format($value->spesifikasi_volume/1000,2,',','.') ?> kg )</option>
                                                <?php
                                                    }
                                                ?>
                                            </select>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#simpan-pilih').click(function (e) { 
            ...
            $.ajax({
                type: "POST",
                url: "<?= base_url().'glow/json_detail_formula' ?>",
                data: {id_bom: id},
                dataType: "json",
                success: function (response) {
                    if (response.length == 0) {
                        ...
                        $.each(response, function (indexInArray, valueOfElement) { 
                            id_detail.push(valueOfElement.id_detail_formula_request);
                            mrow += "<tr id='id"+valueOfElement.id_detail_formula_request+"'>";
        ...
        $('#produk-jadi-msglow').change(function (e) { 
            $('#kode-formula').val('-');
            $('#kode-formula').trigger('change.select2'); 
            ...
            $.ajax({
                type: "POST",
                url: "<?= base_url().'glow/json_bom_request' ?>",
                data: {id_produk: id},
                dataType: "json",
                success: function (response) {
                    if (response.length == 0) {
                        ...
                        $.each(response, function (indexInArray, valueOfElement) { 
                            mbom += "<option value="+valueOfElement.id_formula_msglow+">"+valueOfElement.kode_formula_msglow+"</option>";
                        });
            ...
            if (id_detail.length != 0) {
                id_detail = [];
                $("#data_order").html("<tr><td colspan='6' align='center' class='dataTables_empty'>Tidak ada barang yang dipilih</td></tr>");
            }
